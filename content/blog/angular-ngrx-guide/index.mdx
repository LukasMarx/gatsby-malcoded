---
title: Learn Redux in Angular with NgRx Store (version 4)
image: ./img/redux.png
...
But it doesn't have to be like this. The solution to the problem is called [Redux ]([object Object])and it's angular implementation ngrx& ngrx store.
...
![server-side-state-ngrx](./img/server-side-state-ngrx.png)
...
![distributed-state](./img/distributed-state.png)
...
Redux promises to resolve the issues mentioned above, by saving the state in one single object, called the store. This concept is called Single Source of Truth. 
...
![single-state](./img/single-state.png)
...
["]([object Object])[The only way to change the state is to emit an ]([object Object])[action]([object Object])[, an object describing what happened."]([object Object])

If we want to change the state in any way, we have to make a request in form of an action. The advantage of that is that these actions can be saved and act as a kind of transaction log. That way, we can easily revert or replay an action, e.g when implementing an undo-functionality.

 This also prevents unwanted side-effects, because all changes happen centralized and in a strict order.

Pure Functions
We will later learn, that we can not only make requests in form of actions, but can also react to an action. For that, we will register a reducer. This reducer is a [pure function]([object Object]). That means, it takes the state, makes a copy of it and then applies the changes to the copy.

![pure-function-ngrx](./img/pure-function-ngrx.png)

 It also is not allowed to modify any variable, that is outside of its block-scope.
![impure-function-ngrx](./img/impure-function-ngrx.png)
...
It becomes a little bit more tricky for any "medium" size application. It turns out, Redux is not the only solution for managing state efficiently. There are other principles/frameworks like [MobX]([object Object]), that have a lot less overhead than Redux.
...
```undefined title=undefined
 yarn add @ngrx/core @ngrx/effects @ngrx/store
...
## Changing the State with Reducers 
...
In the beginning of this article I mentioned, that reducers have to be pure functions. So, what is a pure function?
...
For a complete walk-through of the topic, read this [great article about impure functions]([object Object]).
...
Dispatching an action is quite easy. First, we need a reference to our store. We can get one via [dependency injection]([object Object]).
...
Using the state in our components is fortunately quite simple. That is when you are comfortable with RxJs. If not, I recommend rading my [guide on RxJs especially designed for angular developers]([object Object]).

But before we can begin, we need to define some selectors.

Selectors
Selectors are a concept of ngrx-store. They are just small and simple functions, that allow us, to get only the part of the application state, that we are interested in. I've created this functions in the index.ts inside of the reducers directory.
...
The [async pipe]([object Object]) is a little helper angular provides. It allows us, to resolve observables directly from our template's HTML code. It takes care of all the technical difficulties, including unsubscribing from the observable. 
...
```html title=src/app/app.component.html
 ...
 ``` 
Pipes are represented by single vertical line '|'. After that, the name of the pipe follows. With the code above, our input field now always represents the state of "amount". 
...
Last but not least, we have to register our effect in the applications app.module. We also have to import he EffectsModule of ngrx. Also, don't forget to provide our CurrencyService at this point.
