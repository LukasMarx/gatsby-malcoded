...
Probably the most common mistakes, beginners make, is not to import the required [modules]([object Object]). Why? Because they don't even know about them. Of course, they don't. Getting an overview of the angular framework takes some time. Unfortunately, that often results in mysteriously not working angular applications. The errors you get look something like this:

```undefined title=undefined
 Can't bind to 'ngModel' since it isn't a known property of 'input' 
 ``` 
This error indicates, that the [angular Forms Module]([object Object]) has not been imported into your module. 

```undefined title=undefined
 Unhandled Promise rejection: No provider for HttpClient! 
 ``` 
This error tells you, that you have not imported the angular [HttpClient Module]([object Object]) into your (root) module.
...
This advice does not only apply to framework [angular modules]([object Object]). It also applies to any angular module you might want to use, including third-party ones.
...
BrowserAnimationsModule / NoopAnimationsModule
 ...
 ``` 
It is good practice for third party libraries to split up in as many modules as possible, to keep the size of your application small. With [Angular Material]([object Object]), for example, you have to import a module, for every component you use.
...
One example of accessing the DOM when it does not exist is in the constructor of the component. Another one would be in the [ngOnInit]([object Object]) lifecycle callback.
...
Manipulating the DOM in angular directly is not only considered bad practice. It can also result in your angular app not working in a different environment other than the browser. The most popular example is the so-called [Angular Universal]([object Object]) project, which enables your angular application to be rendered server-side. But why should you even care? Read [all about Angular universal and server-side rendering in this step-by-step guide.]([object Object])
...
export class ExampleComponent implements AfterViewInit {
 ...
 ``` 
The Solution
Instead of changing the elements directly, you should manipulate them indirectly. For that, angular offers a rendering API in the form of the [Renderer2]([object Object]) class. Yes, that '2' is intentional and yes there was a [Renderer (1)]([object Object]). Not the best name, but it is what it is.
...
export class ExampleComponent implements AfterViewInit{
 ...
 ``` 
The Renderer2 has many different methods to alter an element. Many of them look very familiar to the JavaScript DOM API. Guessing what they do, should not be the issue. You can find the full list of methods at the [official documentation]([object Object]).

## 4. Avoid duplicate providers overriding each other 

You may have heard that angular utilizes a concept called [dependency injection]([object Object]). With the help of dependency injection, you can request instances of your services in your constructor. 
...
The Solution
Because of that, if you need to protect any sensitive data, you also need to have an actually secure, server-based, solution. For example with signed [JavaScript Web Tokens]([object Object]).

## 6. Declare your Components only once 

For [components ]([object Object])to work in angular, they have to be declared in a module. So as long as we have only one module (AppModule) and we register our components inside of it, there is no problem. 
...
Kudos to Kara Erickson for her findings of that. You can learn more about the topic in [her great article here]([object Object])!
...
As a general advice, it is always good practice, to extract your core business logic into [services]([object Object]). That way, it becomes much easier to maintain, as it can be swapped out and replaced by a new implementation in just a few seconds.
...
Don't believe me? You can analyze the angular bundle itself, by using a tool called [webpack-bundle-analyzer]([object Object]). All you need to do is to create your bundle with the stats-json parameter.
...
To prevent that, make sure to unsubscribe from your subscriptions, when the component is destroyed. One good place to do so, would be the [ngOnDestroy]([object Object]) lifecycle hook. Here is an example:
...
Never miss a post, by following me on twitter [@malcoded]([object Object]).
