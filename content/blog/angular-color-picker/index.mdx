...
To draw all the different colors, we are going to need a [HTML-canvas]([object Object]). We can create one by using the canvas-tag in our template.
...
```html title=src/app/color-picker/color-slider/color-slider.component.html
 <canvas #canvas class="color-slider" width="50" height="250">
</canvas>
 
 ``` 
To use this canvas with angular, we need a reference to the HTML-element. We can get this in a save way by using the angular [@ViewChild]([object Object]) decorator.
...
To do that, we use the [@HostListener]([object Object]) decorator, which is just a fancy way of registering an event-callback in angular.
