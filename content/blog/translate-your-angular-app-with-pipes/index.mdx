...
We all know pipes, don't we? You stuff something in on one end and (hopefully) it comes out at the other end. That's exactly how Angular [pipes ]([object Object])work! 
...
With pipes it is really easy, to format our date right before displaying it on the screen, without changing our [Date]([object Object]) variable in any way.
...
In this example we will store the translation in a Dictionary/Map-like data structure. In a real world scenario, you could easily extend this translation service to get the translation data from your own backend or e.g. the [Google Translate Api]([object Object]).
...
Pipes are marked by the pipe-decorator. To use this decorator, you need to import [Pipe ]([object Object])from '@angular/core'.

Because we will need it later, we import [PipeTransform]([object Object]), as well.

```typescript title=src/translate.pipe.ts
 import { TranslationService } from './translation.service';
import { Pipe, PipeTransform } from '@angular/core'; 
 ``` 
Next, we create and export a new class called 'TranslatePipe' that implements [PipeTransform]([object Object]). Afterwards we decorate it with the pipe decorator, containing it's name and a purity flag.
...
export class TranslatePipe implements PipeTransform {
 ...
 ``` 
In the constructor, we request an instance of our TranslationService via [dependency injection]([object Object]). In the transform method of the pipe, we then call the translate method of the service, to transform the input to the translation. As we don't have any additional arguments, we can ignore the args parameter.
...
The [pure and the impure]([object Object]). 
...
No problem. There are already some nice translation libraries for Angular 2 out there, that cover much more features than this example. You may want to take a look at this Github repo [https://github.com/ngx-translate/core]([object Object]). I haven't tried it myself, but it looks very promising to me.

If you want to stick with my approach, and implement a solution your self, you can take a look at the [full code of this example]([object Object])[ here]([object Object]).
