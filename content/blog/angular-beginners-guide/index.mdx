...
We will start from scratch and build our first angular app.


Afterward, we will take a closer look at all the parts, it consists of, to give you a feeling for the framework.
...
After reading this angular tutorial, you will be able to write your own, fully featured angular applications with ease. You will know about all the critical "components" of such an app and know exactly what they are doing.
...
Thanks to technologies like [Service Workers]([object Object]) and [Progressive Web Apps]([object Object]), these applications can become almost indistinguishable from native applications, especially on mobile (Android) devices. 
...
Angular was originally intended to be used with the programming languages JavaScript,  TypeScript and Dart. And although it is still possible to use it with all of these languages, the by far most popular language of choice became TypeScript. I would highly recommend using TypeScript, as the other versions are not only lacking documentation and community, but updates are also released much later (months not days) than for the TypeScript version.

The Platform
Over time, the angular developers have developed a great ecosystem around the framework. Tools like the Angular-Cli, Angular Universal and Angular Material got added to the project. These tools added great features like fast project generation, server-side rendering, and stunning UI components, that are not coupled with the core-framework itself, but completely optional. 
...
To use the angular-cli, you need to have [Node.js]([object Object]) & [npm ]([object Object])installed on your machine.

You can then install the cli using [npm]([object Object]), by typing the following command into your command line:

```
 npm install -g @angular/cli
 
 ``` 
This tells [npm]([object Object]), to install the angular-cli on your machine. The -g flag is used to install it globally, accessible from every directory on your computer.
...
If you are using Visual Studio Code, I highly recommend the [Angular Files extension]([object Object]). It wraps all these commands behind a nice user interface inside of the editor.
...
```
 ...
 ``` 
The prod-flag will cause the CLI to use [ahead-of-time compilation]([object Object]) (AOT). Additional bloat like source-maps are not included in the build.
...
A [full list of possible options can be found here]([object Object]).
...
If you want to learn more about each option, here is [the complete list]([object Object]).
...
Angular is using [Protractor ]([object Object])for end-to-end testing. Protractor is e2e testing framework, that was built specifically for angular.
...
This is a typescript configuration file, that includes information, how the TypeScript should be compiled to JavaScript. You can read [more about the tsconfig.json here]([object Object]).
...
karma.conf.js
A configuration file for the [Jasmine unit testing framework]([object Object]) and the [Karma test runner]([object Object]).
...
protactor.conf.js
Again, a file to configure a test framework, This time it is for [protractor]([object Object]), the tool used for end-to-end (e2e) testing.
...
You can also disable this behavior using the [encapsulation ]([object Object])option in the components decorator.
...
export class ShinyButtonComponent implements OnInit {
 ...
 ``` 
Notice, that the CLI generated a method called [ngOnInit ]([object Object])for us. This method is a so-called [lifecycle-hook]([object Object]).
...
Since our new input can be treated just like any other html-attribute, we could also interpolate a value into it. You should be careful with that though, as these data-binding-chains become very confusing very fast and very hard to debug. Instead, you should try using a [service]([object Object]).
...
For that, we need to create a new property of the type [EventEmitter]([object Object]). The EventEmitter is a class of the angular framework and represents our communication-channel to the outside world (of our component). We can pass messages (objects) into it, that are then delivered to the listening parent component and it's assigned callback.
...
```typescript title=src/app/shiny-button/shiny-button.component.ts
 ...
 ``` 
Next, we create a new property called colorEventEmitter and decorate it with the @Output decorator. This property is of type EventEmitter<string>, because we want to send the new color (string) to our parent-component. We initialize the property right there, as well.
...
<ng-template #elseBlock>else example</ng-template>
 ...
 ``` 
Switch-Directive
Are you familiar with the switch-statement e.g. in JavaScript. We can have that in our template, as well, by using the [ngSwitch-directive]([object Object]). Because I think using that directive is not very common, I will just show what it looks like, so you know that it exists. Personally, I have never used this directive.
...
```html title=null
 ...
 ``` 
This example is taken from the [official documentation]([object Object]).
...
```html title=null
 ...
 ``` 
If you want to [learn how to create your own structural directive, you can learn more about that in this great article about that]([object Object]).
...
The first one is the [ngClass-directive]([object Object]). With the help of this directive, you can map css-classes to properties of your component. This makes it very easy to toggle css classes in your templates.
...
providers
We haven't talked about services and [dependency injection]([object Object]), yet, but let's say you can services, that should be accessible in your module, here. They are then provided via dependency injection to every component in your module. 

These are the most common options. See [all module-decorator-options]([object Object]) here.
...
Services are also created as [singletons]([object Object]), making them a great solution for inter-component-communication. 
...
export class ExampleComponent implements OnInit {
 ...
 ``` 
For more details, you can read my [article about dependency injection]([object Object]).
...
[HTTP in Angular]([object Object])

[Angular Services]([object Object])

[Routing with the Angular Router]([object Object])

[Angular Modules]([object Object])

[Dependency Injection in Angular]([object Object])
