...
That is because they can be easily requested via [Dependency Injection]([object Object]). 
...
Use Cases
The most common use case is I/O (Input/Output). To get more specific: HTTP requests. Generally, all HTTP requests in Angular are wrapped by a service. Why? Because with the help of [Dependency Injection]([object Object]), our code stays highly maintainable. Here is an example:
...
export class CachedHttpService {
 ...
 ``` 
Every class (or service) served via [Dependency Injection]([object Object]), has to be provided somewhere. We do so in our AppModule. We also need to import the HttpClientModule here, if we actually want to make any requests. 
...
To get informed about new content, go ahead and follow me on twitter [@malcoded]([object Object]).
